...
- `CameraManager.swift` - `@MainActor @Observable` owner of the `AVCaptureSession`
  *and* of movie capture (`startRecording()` / `stopRecording()`).
  ...
  with `automaticallyAdjustsVideoMirroring` switched back on. The split is the
  point: a mirror is what you want while framing yourself, but a mirrored file
  has all its text backwards, which is what turning mirroring off on 2026-09-10
  fixed. `applyMirroring` therefore **skips the movie output's connection**, so
  flipping the toggle can never disturb a file being written -- which is why the
  toggle has no disabled state and can be flipped as often as the user likes.
  It also sets the preview layer's connection explicitly rather than trusting
  it to appear in `session.connections`, since if it ever did not the toggle
  would silently do nothing. `applyMirroring` is
