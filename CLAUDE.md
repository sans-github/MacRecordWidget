...
- `MacVoiceRecordWidgetApp.swift` - SwiftUI `@main` entry point, `.window`-style `MenuBarExtra` (popover panel). Contains a `Toggle` switch for video mode, bordered Start/Stop buttons, and Quit. Menu bar icon switches between `mic`/`mic.fill` (audio-only) and `video`/`video.fill` (video on), green while recording.
