...
| **Mirror toggle** (flip icon) | Flips the preview left to right. Off by default, and the setting persists across launches. It affects the preview only: recordings are always saved unmirrored, so text you hold up to the camera reads correctly in the file. Free to flip at any time, recording or not. |
